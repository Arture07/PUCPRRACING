...
def configurar_estilo_plot(ax: plt.Axes, title: str = "Gráfico"):
    ...
    ax.grid(True, linestyle="--", alpha=0.4, color=COLOR_TEXT)
    ax.set_title(title, color=COLOR_TEXT)


def plotar_dados_no_canvas(
    df: Optional[pd.DataFrame],
    channels: List[str],
    canvas_widget: Optional[Any],
    fig: Figure,
    ax: plt.Axes,
):
        ...
        if valid_channels:
            for channel in valid_channels:
                try:
                    ...
                    print(f"Erro plot {channel}: {e}")
                    ax.text(0.5, 0.5, f"Erro {channel}", ha="center", color=COLOR_RED)
                    break
            else:
                ax.legend(fontsize="small", labelcolor=COLOR_TEXT)
                ax.set_xlabel("Tempo")
                ax.set_ylabel("Valor")
            ...
            ax.text(
                0.5,
                0.5,
                "Nenhum canal válido selecionado",
                ha="center",
                color=COLOR_TEXT,
            )
    else:
        ax.text(
            0.5, 0.5, "Carregue dados e selecione canais", ha="center", color=COLOR_TEXT
        )
    try:
        fig.tight_layout()
    except ValueError:
        print("Aviso: Erro no fig.tight_layout().")
    if canvas_widget is not None:
        canvas_widget.draw()


def plotar_gg_diagrama_nos_eixos(
    gg_data: pd.DataFrame,
    canvas_widget: Optional[Any],
    fig: Figure,
    ax: plt.Axes,
    lat_accel_col: Optional[str],
    lon_accel_col: Optional[str],
):
    ...
    if not gg_data.empty and lat_accel_col and lon_accel_col:
        try:
            ax.scatter(
                gg_data[lat_accel_col],
                gg_data[lon_accel_col],
                alpha=0.4,
                s=5,
                color=COLOR_GOLD,
            )
            ax.set_xlabel(f"{lat_accel_col} (G)")
            ax.set_ylabel(f"{lon_accel_col} (G)")
            ax.axhline(0, color=COLOR_TEXT, linewidth=0.5)
            ax.axvline(0, color=COLOR_TEXT, linewidth=0.5)
            ax.axis("equal")
            x_max = gg_data[lat_accel_col].abs().max() * 1.1
            y_max = gg_data[lon_accel_col].abs().max() * 1.1
            lim = max(x_max, y_max, 0.1)
            ax.set_xlim(-lim, lim)
            ax.set_ylim(-lim, lim)
        except Exception as e:
            ax.text(
...
def plotar_mapa_pista_nos_eixos(
    df: Optional[pd.DataFrame],
    canvas_widget: Optional[Any],
    fig: Figure,
    ax: plt.Axes,
    ...
    configurar_estilo_plot(
        ax, f"Mapa da Pista {('- Cor: ' + color_channel) if color_channel else ''}"
    )
    if (
        df is not None
        and lat_col
        ...
        try:
            ...
            lat_data = lat_data.loc[common_index]
            lon_data = lon_data.loc[common_index]
            if not lat_data.empty:
                scatter_color = COLOR_GOLD
                c_data = None
                cmap = "plasma"
                color_col_real = (
                    color_channel  # Usa direto o nome da coluna selecionado no combo
                )
                if color_col_real and color_col_real in df.columns:
                    ...
                    if not final_index.empty:
                        lat_data = lat_data.loc[final_index]
                        lon_data = lon_data.loc[final_index]
                        c_data = c_data_series.loc[final_index]
                        scatter_color = c_data
                    else:
                        print(
                            f"Aviso: Sem dados válidos para colorir com {color_col_real}"
                        )
                        color_channel = None
                sc = ax.scatter(
                    lon_data, lat_data, c=scatter_color, cmap=cmap, s=5, marker="."
                )
                ax.set_xlabel(f"{lon_col} (Longitude)")
                ax.set_ylabel(f"{lat_col} (Latitude)")
                ax.axis("equal")
                if color_channel and c_data is not None:
                    cbar = fig.colorbar(sc, ax=ax, fraction=0.046, pad=0.04)
                    cbar_label = color_channel
                    cbar.set_label(cbar_label, color=COLOR_TEXT)
                ...
                ax.text(
                    0.5, 0.5, "Dados GPS insuficientes", ha="center", color=COLOR_TEXT
                )
        except Exception as e:
            ax.text(
                0.5, 0.5, f"Erro ao plotar Mapa:\n{e}", ha="center", color=COLOR_RED
            )
    else:
        ax.text(
            0.5,
            0.5,
            f"Colunas GPS ('{lat_col}', '{lon_col}') não encontradas",
            ha="center",
            color=COLOR_TEXT,
        )
    try:
...
def plotar_analise_skidpad(
    ...
    lat_col = get_channel_name(
        config_map, "lataccel", df.columns if df is not None else None
    )
    configurar_estilo_plot(ax, "Análise Skid Pad (Placeholder)")
    if df is not None and lat_col:
        try:
            ax.plot(
                df.index, df[lat_col], label=f"{lat_col} (Lat Accel)", color=COLOR_GOLD
            )
            ax.set_xlabel("Tempo")
            ax.set_ylabel("Aceleração Lateral (G)")
        ...
        ax.text(
            0.5,
            0.5,
            f"Coluna LatAccel não encontrada/mapeada.",
            ha="center",
            color=COLOR_TEXT,
        )
    try:
...
def plotar_analise_aceleracao(
    ...
    if df is not None:
        ...
        if vehicle_speed and vehicle_speed in df.columns:
            ...
            speed_data_to_plot = df[speed_col]
            speed_col_name = f"{speed_col} (mapeada)"
        elif ws_fl and ws_fr:
            try:
                speed_data_to_plot = df[[ws_fl, ws_fr]].mean(axis=1)
    ...
    if df is not None and speed_data_to_plot is not None:
        ...
        ax.text(
            0.5,
            0.5,
            "Nenhuma coluna de velocidade encontrada.",
            ha="center",
            color=COLOR_TEXT,
        )
    try:
...
def plotar_histograma_suspensao(
    df: Optional[pd.DataFrame],
    canvas_widget: Optional[Any],
    fig: Figure,
    ax: plt.Axes,
    ...
    if df is not None:
        susp_cols_real = [
            get_channel_name(config_map, n, df.columns) for n in susp_internal
        ]
        susp_cols_real = [
            c for c in susp_cols_real if c is not None and c in df.columns
        ]
    ...
    if df is not None and susp_cols_real:
        plotted = False
        try:
            for col in susp_cols_real:
                ax.hist(df[col].dropna(), bins=30, alpha=0.7, label=col)
                plotted = True
            if plotted:
                ax.legend(labelcolor=COLOR_TEXT)
                ...
                ax.text(
                    0.5,
                    0.5,
                    "Colunas de suspensão não encontradas",
                    ha="center",
                    color=COLOR_TEXT,
                )
            ax.relim()
            ...
            ax.text(
                0.5,
                0.5,
                f"Erro ao plotar Histograma:\n{e}",
                ha="center",
                color=COLOR_RED,
            )
    else:
        ax.text(
            0.5, 0.5, "Carregue dados/mapeie colunas", ha="center", color=COLOR_TEXT
        )
    try:
        fig.tight_layout()
    except ValueError:
...
def plotar_delta_time(
    ...
    ax.text(
        0.5,
        0.5,
        "Cálculo e Plotagem de Delta-Time\nAinda Não Implementado",
        ha="center",
        va="center",
        color=COLOR_TEXT,
    )
