...
def calcular_metricas_gg(
    df: Optional[pd.DataFrame], config_map: Dict[str, str]
) -> Tuple[pd.DataFrame, Optional[str], Optional[str], Optional[str]]:
    """Prepara dados para o diagrama G-G."""
    if df is None:
        return pd.DataFrame(), None, None, "Erro: Log não carregado."
    lat_col = get_channel_name(config_map, "lataccel", df.columns)
    lon_col = get_channel_name(config_map, "lonaccel", df.columns)
    error_msg = ""
    if not lat_col:
        error_msg += "- Coluna LatAccel (lataccel) não encontrada/mapeada.\n"
    if not lon_col:
        error_msg += f"- Coluna LonAccel (lonaccel) não encontrada/mapeada.\n"
    if error_msg:
        return pd.DataFrame(), None, None, f"Erro:\n{error_msg}"
    try:
...
def calcular_tempos_volta(
    ...
    global frequency
    if df is None:
        return None, "Erro: Log não carregado."
    ...
    error_msg = ""
    if not lat_col:
        error_msg += "- Coluna GPS Latitude (gpslat) não encontrada/mapeada.\n"
    if not lon_col:
        error_msg += "- Coluna GPS Longitude (gpslon) não encontrada/mapeada.\n"
    has_time_index = isinstance(df.index, pd.DatetimeIndex)
    has_time_col = (
        time_col
        and time_col in df.columns
        and pd.api.types.is_numeric_dtype(df[time_col])
    )
    ...
    if error_msg:
        return None, f"Erro: Dados insuficientes:\n{error_msg}"

    try:
        start_finish_lat = float(track_config.get("startfinishlat", 0.0))
        start_finish_lon = float(track_config.get("startfinishlon", 0.0))
        threshold_m = float(
            analysis_config.get("lapdetectionthresholdmeters", 15.0)
        )  # Default 15m
        min_lap_time_s = float(
            analysis_config.get("minlaptimeseconds", 20.0)
        )  # Default 20s
        print(
            f"Calculando Voltas: Linha em ({start_finish_lat}, {start_finish_lon}), Threshold: {threshold_m}m, Tempo Mín: {min_lap_time_s}s"
        )

        # --- Lógica de Tempo ---
        time_seconds = None
        if has_time_index:
            time_deltas = df.index - df.index[0]
            time_seconds = pd.Series(time_deltas.total_seconds(), index=df.index)
        elif has_time_col:
            time_seconds = df[time_col] - df[time_col].iloc[0]
            time_seconds = time_seconds.fillna(0)
        else:
            time_seconds = pd.Series(df.index * (1 / frequency), index=df.index)
            print(f"Aviso: Usando índice numérico (freq={frequency} Hz).")
        if time_seconds is None:
            return None, "Erro: Tempo decorrido não determinado."
        time_seconds.name = "elapsed_seconds"

        # --- Lógica de Detecção de Cruzamento ---
        print("Calculando distâncias da linha de chegada...")
        distances = haversine(
            df[lat_col].values, df[lon_col].values, start_finish_lat, start_finish_lon
        )
        ...
        if crossing_times:
            print(
                f"Tempos dos cruzamentos detectados (s): {[f'{t:.2f}' for t in crossing_times]}"
            )
        # --- FIM DEBUG PRINTS ---
        ...
        if len(crossing_times) < 2:
            return (
                None,
                "Não foram detectados cruzamentos suficientes da linha de chegada (mínimo 2).",
            )
        ...
        for k in range(1, len(crossing_times)):
            ...
            if time_diff >= min_lap_time_s:
                ...
                print(
                    f"  -> Cruzamento em {current_crossing_time:.2f}s ignorado (tempo desde último válido: {time_diff:.2f}s < {min_lap_time_s}s)"
                )
        ...
        for lap_idx, cross_time_start in enumerate(valid_crossing_times):
            if lap_idx + 1 < len(valid_crossing_times):
                ...
                lap_mask = (time_seconds >= cross_time_start) & (
                    time_seconds < cross_time_end
                )
            else:
                lap_mask = time_seconds >= cross_time_start
        ...
        if not lap_times:
            return (
                lap_numbers,
                "Nenhuma volta completa detectada (tempo mínimo não atingido entre cruzamentos válidos).",
            )
...
def calcular_metricas_skidpad(
    df: Optional[pd.DataFrame], config_map: Dict[str, str]
) -> str:
    """Calcula métricas básicas do Skid Pad."""
    if df is None:
        return "Erro: Log não carregado."
    lat_col = get_channel_name(config_map, "lataccel", df.columns)
    if not lat_col:
        return "Erro: Canal LatAccel (lataccel) não mapeado."
    try:
...
def calcular_metricas_aceleracao(
    df: Optional[pd.DataFrame],
    config_map: Dict[str, str],
    distance_target: float = 75.0,
) -> str:
    """
    ...
    if vehicle_speed_col and vehicle_speed_col in df.columns:
        speed_col = vehicle_speed_col
        speed_data = df[speed_col]
        speed_col_name = f"{speed_col} (mapeada)"
    elif ws_fl and ws_fr:
        try:
            speed_data = df[[ws_fl, ws_fr]].mean(axis=1)
            speed_col = f"Média Rodas ({ws_fl}, {ws_fr})"
            speed_col_name = speed_col
        except Exception:
            speed_data = None
    if speed_data is None and gps_speed and gps_speed in df.columns:
        speed_col = gps_speed
        speed_data = df[speed_col]
        speed_col_name = f"{speed_col} (GPS)"

    # Verifica se encontrou velocidade e tempo
    error_msg = ""
    if speed_data is None:
        error_msg += f"- Nenhum canal de velocidade válido encontrado/mapeado.\n"
    has_time_index = isinstance(df.index, pd.DatetimeIndex)
    has_time_col = (
        time_col
        and time_col in df.columns
    ...
    if not has_time_index and not has_time_col:
        error_msg += f"- Coluna Timestamp ('{time_col or 'timestamp'}') não encontrada/inválida.\n"
    if not throttle_col:
        error_msg += f"- Coluna ThrottlePos ('{throttle_col or 'throttlepos'}') não encontrada/mapeada.\n"
    if error_msg:
        return f"Erro: Dados insuficientes para Aceleração:\n{error_msg}"

    print(
        f"Calculando Aceleração: Usando Velocidade='{speed_col_name}', Throttle='{throttle_col}', Tempo='{time_col or 'Index'}'"
    )
    ...
    try:
        ...
        if has_time_index:
            ...
            print(f"Aviso: Usando índice numérico (freq={frequency} Hz).")
        if time_seconds is None:
            return "Erro: Tempo decorrido não determinado."
        time_seconds.name = "elapsed_seconds"
        ...
        if not start_mask.any():
            print(
                "Aviso: Início da aceleração não detectado claramente (Vel < 1 e Thr > 80). Começando do início do log."
            )
        ...
        time_run_relative = (
            time_run - time_run.iloc[0]
        )  # Tempo relativo ao início da puxada

        # --- Calcula Distância Percorrida ---
        # Integração trapezoidal simples: dist += V_media * dt
        # V_media = (V_atual + V_anterior) / 2
        distance_run = (
            (speed_run.shift(fill_value=0) + speed_run) / 2 * dt_run
        ).cumsum()
            ...
            try:
                ...
                if idx_pos > 0:
                    ...
                    if dist_curr > dist_prev:  # Evita divisão por zero
                        interp_factor = (distance_target - dist_prev) / (
                            dist_curr - dist_prev
                        )
                        time_to_target = time_prev + interp_factor * (
                            time_curr - time_prev
                        )
        ...
        if pd.isna(time_to_target):
            result_text = (
                f"Distância alvo ({distance_target}m) não atingida.\n"
                f"Velocidade Máx (durante tentativa): {max_speed_run:.1f} m/s (usando {speed_col_name})"
            )
        else:
            result_text = (
                f"Tempo para {distance_target:.0f}m: {time_to_target:.3f} s\n"
