...
def carregar_log_csv(
    filepath: str, config_map: Dict[str, str]
    ...
    try:
        ...
        potential_time_cols = [
            col for col in df.columns if col.lower() == timestamp_log_name.lower()
        ]
            ...
            try:
                ...
                if numeric_time.notna().sum() > len(df) * 0.5:  # Maioria numérica?
                    ...
                    df["Timestamp_dt"] = pd.to_datetime(
                        numeric_time, unit="s", errors="coerce"
                    )
                ...
                if (
                    "Timestamp_dt" in df.columns
                    and df["Timestamp_dt"].notna().sum() > len(df) * 0.8
                ):
                ...
                print(
                    f"Aviso: Não foi possível converter '{timestamp_col}' para índice: {e}"
                )
                if "Timestamp_dt" in df.columns:
                    df = df.drop(columns=["Timestamp_dt"])
        ...
        if not time_col_found:
            print(
                "Aviso: Nenhuma coluna de timestamp válida encontrada/convertida. Usando índice numérico padrão."
            )
            # Garante que a coluna original (se existir) seja numérica
            if potential_time_cols and potential_time_cols[0] in df.columns:
                df[potential_time_cols[0]] = pd.to_numeric(
                    df[potential_time_cols[0]], errors="coerce"
                )

        print(f"Dados carregados. Colunas disponíveis: {df.columns.tolist()}")
        return df
    except FileNotFoundError:
        # Em contexto GUI, exibe messagebox; no web, preferir função dedicada abaixo
...
def carregar_log_csv_web(filepath: str, config_map: Dict[str, str]) -> Optional[pd.DataFrame]:
    """Versão silenciosa para uso web (sem messagebox)."""
    if not filepath:
        return None
    try:
        return carregar_log_csv(filepath, config_map)
    except Exception:
        # Não exibe messagebox no ambiente web
        return None
